...
function read(docxFile, input) {
    ...
    return promises.props({
        ...
        return {
            ...
            commentsExtended: readXmlFileWithBody(result.partPaths.commentsExtended, result, function(bodyReader, xml) {
                if (xml) {
                    return commentsExtendedReader.createCommentsExtendedReader(bodyReader)(xml);
                } else {
                    return new Result([]);
                }
            })
        ...
        return readXmlFileWithBody(result.partPaths.mainDocument, result, function(bodyReader, xml) {
            return result.notes.flatMap(function(notes) {
                return result.comments.flatMap(function(comments) {
                    if (result.commentsExtended && result.commentsExtended.value && result.commentsExtended.value.length > 0) {
                        return result.commentsExtended.flatMap(function(commentsExtended) {
