...
function BodyReader(options) {
    ...
    function readXmlElements(elements, style) {
        var results = elements.map(function(el) {
            return readXmlElement(el, style);
        });
        var combinedResults = combineResults(results);
        return combinedResults;
    }

    function readXmlElement(element, style) {
        if (element.type === "element") {
            var handler = xmlElementReaders[element.name];
            if (handler) {
                return handler(element, style);
    ...
    function readParagraphProperties(element, pElement) {
        return readParagraphStyle(element).map(function(style) {
            return {
                ...
                paraId: pElement.attributes["{http://schemas.microsoft.com/office/word/2010/wordml}paraId"]
            };
        });
    }
    ...
    function readRunProperties(element, parentStyleResult) {
        return readRunStyle(element).map(function(style) {
            var fontSizeString = element.firstOrEmpty("w:sz").attributes["w:val"];
            // w:sz gives the font size in half points, so halve the value to get the size in points
            var fontSize = /^[0-9]+$/.test(fontSizeString) ? parseInt(fontSizeString, 10) / 2 : null;
            var parentStyles = parentStyleResult && parentStyleResult._result &&
                parentStyleResult._result.value && parentStyleResult._result.value.element
                ? parentStyleResult._result.value.element : {};
            ...
            return {
                ...
                font: element.firstOrEmpty("w:rFonts").attributes["w:ascii"] || style.font || parentStyles.font,
                fontSize: fontSize,
                isBold: element.first("w:b") ? readBooleanElement(element.first("w:b")) : readBooleanElement(parentStyles.bold),
                isUnderline: element.first("w:u") ? readUnderline(element.first("w:u")) : readUnderline(parentStyles.underline),
                isItalic: element.first("w:i") ? readBooleanElement(element.first("w:i")) : readBooleanElement(parentStyles.italic),
                isStrikethrough: readBooleanElement(element.first("w:strike")),
                isAllCaps: readBooleanElement(element.first("w:caps")),
                isSmallCaps: readBooleanElement(element.first("w:smallCaps")),
                highlight: readHighlightValue(element.firstOrEmpty("w:highlight").attributes["w:val"]),
                fill: readColorValue(element.firstOrEmpty("w:shd").attributes["w:fill"]),
                color: readColorValue(element.firstOrEmpty("w:color").attributes["w:val"])
            };
        });
    }

    function readEditAttr(element) {
        var editAttr = {};
        if (element && (element.name === 'w:del' || element.name === 'w:ins') && element.attributes) {
            if (element.name === 'w:del') {
                editAttr['isDelete'] = true;
            }
            if (element.name === 'w:ins') {
                editAttr['isInsert'] = true;
            }
            if (element.attributes['w:author']) {
                editAttr['editAuthor'] = element.attributes['w:author'];
            }
            if (element.attributes['w:id']) {
                editAttr['editId'] = element.attributes['w:id'];
            }
            if (element.attributes['w:date']) {
                editAttr['editDate'] = element.attributes['w:date'];
            }
        }
        return editAttr;
    }
    ...
    function readParagraphStyle(element) {
        // console.log('p style results', readStyle(element, "w:pStyle", "Paragraph", styles.findParagraphStyleById)._result);
        ...
        if (styleElement) {
            ...
            if (styleId) {
                ...
                if (style) {
                    ...
                    return elementResultWithMessages({styleId: styleId, name: name, font: null}, messages);
                }
            } else {
                return elementResultWithMessages({styleId: styleId, name: name, font: font}, messages);
            }
        } else {
            return elementResultWithMessages({styleId: styleId, name: name, font: font}, messages);
        }
        // return elementResultWithMessages({styleId: styleId, name: name, font: undefined}, messages);
    }
    ...
    function readChildElements(element, styles) {
    ...
    var xmlElementReaders = {
        "w:p": function(element) {
            ...
            return ReadResult.map(
                readParagraphProperties(paragraphPropertiesElement, element),
                readXmlElements(childrenXml, readParagraphStyle(paragraphPropertiesElement)),
                function(properties, children) {
                    return new documents.Paragraph(children, properties);
                }
            ).insertExtra();
        },
        "w:r": function(element, style) {
            return ReadResult.map(
                readRunProperties(element.firstOrEmpty("w:rPr"), style),
        ...
        "w:ins": function(element) {
            var runProps = readRunProperties(element.children[0]);
            var editAttr = readEditAttr(element);
            Object.keys(editAttr).forEach(function(key) {
                runProps.value[key] = editAttr[key];
            });
            return ReadResult.map(
                runProps,
                readXmlElements(element.children),
    ...
    function readBlip(element, blip) {
        ...
        var extent = element.first("wp:extent") ? element.first("wp:extent").attributes : undefined;
        var altText = isBlank(properties.descr) ? properties.title : properties.descr;
        var blipImageFile = findBlipImageFile(blip);
        if (blipImageFile === null) {
            return emptyResultWithMessages([warning("Could not find image file for a:blip element")]);
        } else {
            return readImage(blipImageFile, altText, extent);
        }
    }
    ...
    function readImage(imageFile, altText, extent) {
        ...
        var image = documents.Image({
            ...
            // Convert EMU to pixels and assume 96 dpi
            // https://stackoverflow.com/questions/66541210/convert-google-slides-emu-units-to-pixels-api
            width: extent && extent.cx ? extent.cx / 9525 : undefined,
            height: extent && extent.cy ? extent.cy / 9525 : undefined
        });
