...
function DocumentConversion(options, comments) {
    ...
    function elementToHtml(element, messages, options) {
        ...
        var handler;
        if (element) {
            handler = elementConverters[element.type];
        } else {
            messages.push('ElementToHtml with no element');
        }
        if (handler) {
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.color !== null) {
            var colorPath = findHtmlPath({type: "color", color: run.color});
            if (colorPath) {
        ...
        // TODO maybe should move this stuff into a seperate list instead of adding as attributes
        if (run.isDelete) {
            paths.push(htmlPaths.element("del", {
                author: run.editAuthor,
                id: 'del' + run.editId,
                date: run.editDate
            }, {fresh: false}));
        }
        if (run.isInsert) {
            paths.push(htmlPaths.element("ins", {
                author: run.editAuthor,
                id: 'ins' + run.editId,
                date: run.editDate
            }, {fresh: false}));
    ...
    function convertCommentReference(reference, messages, options) {
        return findHtmlPath(reference, htmlPaths.ignore).wrap(function() {
            ...
            referencedComments.push({label: label, comment: comment, done: comment.done});
    ...
    function convertComment(referencedComment, messages, options) {
        ...
        var body = convertElements(comment.body, messages, options).concat([
            Html.nonFreshElement("p", {}, [
                Html.text(comment.done === "1" ? "resolved" : " "),
